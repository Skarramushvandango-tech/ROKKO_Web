  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-6 text-[color:var(--text)]">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        {artist.releases && Object.entries(artist.releases).map(([releaseName, rel]) => {
          const isOpen = open === releaseName;
          return (
            <article key={releaseName} className="rounded border border-white/10 bg-black/20 p-3">
              <button
                onClick={() => setOpen(isOpen ? null : releaseName)}
                className="w-full text-left"
                aria-expanded={isOpen}
                title={`${artist.name} – ${releaseName}`}
              >
                <div className="aspect-square overflow-hidden rounded-md border border-white/10 bg-black/20">
                  <img
                    ...
                    alt={`${artist.name} – ${releaseName} (Thumbnail)`}
                    className="h-full w-full object-cover"
                    loading="lazy"
                  />
                </div>
                <div className="mt-2 text-sm font-semibold">{releaseName}</div>
              </button>
              ...
              {isOpen && (
                <div className="mt-3 rounded-md border border-white/10 bg-black/30 p-3">
                  <img
                    src={asset(rel.cover)}
                    alt={`${artist.name} – ${releaseName} (Cover)`}
                    className="w-full h-auto rounded"
                  />
                  <ul className="mt-3 space-y-2">
                    {rel.tracks.map((t, i) => {
                      const k = `${releaseName}-t${i}`;
